...
void VaccinationStatus::addVaccine(const string &v)
{
    ...
    for (int i = 0; i < VACCINE_TABLE_SIZE; ++i) {
        int index = (hash + i * i) % VACCINE_TABLE_SIZE;

      
        if (vaccineHashTable[index].empty()) {
            vaccineHashTable[index] = v; 
            ++numVaccines;
            return;
        } 
        
        if (vaccineHashTable[index] == v) {
...
bool VaccinationStatus::hasVaccine(const string &v) const
{
    ...
    for(int i = 0; i < VACCINE_TABLE_SIZE; i++){
        ...
        if(vaccineHashTable[index] == " "){
            return false;
        }else if(vaccineHashTable[index] == v){
            return true;
        }
    }
    return false;
